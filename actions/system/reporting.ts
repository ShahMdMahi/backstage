...
import { Report, Reporting, User } from "@/lib/prisma/client";
import {
  ROLE,
...
export async function deleteReporting(reportingId: string): Promise<{
  success: boolean;
  message: string;
  data: Partial<Reporting> | null;
  errors: unknown;
  ...
  try {
    ...
    if (session.data.user.role === ROLE.SYSTEM_USER) {
      ...
      if (
        ...
        return {
          success: false,
          message: "User does not have permission to delete reporting.",
          data: null,
          errors: new Error("Insufficient permissions"),
        };
      }
    ...
    if (!deletedReporting) {
      return {
        success: false,
        message: "Failed to delete reporting.",
        data: null,
        errors: new Error("Reporting deletion failed"),
      };
    }

    try {
      await logAuditEvent({
        action: AUDIT_LOG_ACTION.REPORTING_DELETED,
        entity: AUDIT_LOG_ENTITY.REPORTING,
        entityId: deletedReporting.id,
        description: `Reporting "${deletedReporting.name}" deleted by user "${session.data.user.email}".`,
        metadata: { deviceInfo: JSON.stringify(deviceInfo) },
        user: { connect: { id: session.data.userId } },
      });
    } catch (error) {
      console.error("Error logging audit event for reporting deletion:", error);
    }

    return {
      success: true,
      message: "Reporting deleted successfully.",
      data: deletedReporting,
      errors: null,
    };
  } catch (error) {
    console.error("Error fetching reporting:", error);
